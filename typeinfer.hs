...
data MetaType = Mvar Int Bool | Mfun MetaType MetaType
...
setTypeVar :: Int -> Expr -> Map.Map String Int -> [(String,Int)]-> (Aexpr,Int,(Map.Map String Int))
setTypeVar tv (Evar x) mapper list =
   let   
         mb = List.lookup x list
         mb1 = Map.lookup x mapper
         mapper_updated = if ((isNothing mb) && (isNothing mb1)) then Map.insert x tv mapper else mapper
         typ = if (isNothing mb) 
               then if (isJust mb1)
                    then fromJust mb1
                    else tv
               else (fromJust mb)
         tv_next = if ((isNothing mb) && (isNothing mb1)) then tv+1 else tv
   in    ((Avar x (Tvar typ)),tv_next,mapper_updated)
setTypeVar tv (Eabs x e) mapper list =
   let   mapperUpdated = Map.insert x tv mapper
         (ae,ntv,mapfinal) = setTypeVar (tv+1) e mapperUpdated ([(x,tv)] ++ list)
   in    ((Aabs x ae (Tfun (Tvar tv) (type_of ae))),ntv,mapfinal)
setTypeVar tv (Eapp e1 e2) mapEtoT list =
   let   (ae1,tv1,map1) = setTypeVar (tv+2) e1 mapEtoT list
         (ae2,tv2,map2) = setTypeVar tv1 e2 map1 list
   in    ((Aapp ae1 ae2 (Tvar (tv+1))),tv2,map2)
...
typeToMeta :: Type -> Bool -> MetaType
typeToMeta (Tvar x) flag = Mvar x flag
typeToMeta (Tfun a b) flag = Mfun (typeToMeta a flag) (typeToMeta b flag)

metaToType :: MetaType -> Type
metaToType (Mvar x _) = Tvar x
metaToType (Mfun a b) = Tfun (metaToType a) (metaToType b)

--Substitute type t1 for all var str in type t2
metaSubstitute :: Int -> Type -> MetaType -> MetaType
metaSubstitute var t1 t2@(Mvar x flag) = if ((var==x) && flag==False) then (typeToMeta t1 True) else t2
metaSubstitute var t1 t2@(Mfun x y) = (Mfun (metaSubstitute var t1 x) (metaSubstitute var t1 y))

metaApply :: Substitution -> MetaType -> MetaType
metaApply sub typ = Prelude.foldr (\(x,y) -> metaSubstitute x y) typ sub 
...
listOfVariables :: Type -> [Int] -> [Int]
listOfVariables (Tvar x) list = if (x `elem` list) then list else (list ++ [x])
listOfVariables (Tfun a b) list = let l1 = listOfVariables a list
                                   in  listOfVariables b l1
...
reorderingConstraints :: (Int,[Int],Substitution) -> (Int,[Int],Substitution)
reorderingConstraints (c,[],sub) = (c,[],sub)
reorderingConstraints (c,x:xs,sub) =   let newsub = sub ++ [(x,Tvar c)]  
                                       in reorderingConstraints (c+1,xs,newsub)
...
readOne  =  do  s <- getLine
                ...
                if (isNothing subt) then putStrLn "type error"
                     ...
                     let  tmp1 = apply subt (type_of aexp)
                          set = Set.empty
                          uv = getUsedVariables tmp1 set
                          (_,_,subt') = renamingConstraints (0,uv,[])
                          tmp2 = apply (Just subt') tmp1
                          x =listOfVariables tmp2 []
                          (_,_,reorder_sub) = reorderingConstraints (0,x, [])
                          mt = typeToMeta tmp2 False
                          meta_t = metaApply reorder_sub mt
                          output = metaToType meta_t
